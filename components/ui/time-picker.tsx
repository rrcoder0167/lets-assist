...
export function TimePicker({ value, onChangeAction, label, error, errorMessage }: TimePickerProps) {
  const parseTime = (timeString: string) => {
    ...
    try {
      ...
      if (isNaN(hour) || isNaN(minute)) return { hour: 9, minute: 0, period: "AM" as const }
    ...
    <div className="flex flex-col gap-2">
      {label && (
        <div className="flex justify-between items-baseline">
          <Label>{label}</Label>
          {error && errorMessage && (
            <span className="text-xs text-destructive animate-in fade-in">{errorMessage}</span>
          )}
        </div>
      )}
      <Popover>
